...
public class GraficoIdhRenda implements DatasetProducer, ChartPostProcessor {
...
//        ds.addValue(idhGeral, PRODUCER_ATTRIBUTE_NAME, PRODUCER_ATTRIBUTE_NAME);
        for (Indice indice : indices) {
            ds.addValue(indice.getIdhRenda(), municipio.getNomeMunicipio(), String.valueOf(indice.getAno()));
            List<Double> indiceEstado = estado.calcularIDHEstado(indice.getAno());
            ds.addValue(indiceEstado.get(1), estado.getNomeEstado(), String.valueOf(indice.getAno()));
            List<Double>indiceRegiao = regiao.calcularIDHRegiao(indice.getAno());
            ds.addValue(indiceRegiao.get(1), regiao.getNomeRegiao(), String.valueOf(indice.getAno()));
            List<Double>indicePais = controle.calcularIDHTotalPais(indice.getAno());
            ds.addValue(indicePais.get(1),"Brasil", String.valueOf(indice.getAno()));
        }
        return ds;
