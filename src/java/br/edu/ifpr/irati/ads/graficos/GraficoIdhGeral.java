...
public class GraficoIdhGeral implements DatasetProducer, ChartPostProcessor {
...
//        ds.addValue(idhGeral, PRODUCER_ATTRIBUTE_NAME, PRODUCER_ATTRIBUTE_NAME);
        for (Indice indice : indices) {
            ds.addValue(indice.getIdhGeral(), municipio.getNomeMunicipio(), String.valueOf(indice.getAno()));
            List<Double> indiceEstado = estado.calcularIDHEstado(indice.getAno());
            ds.addValue(indiceEstado.get(0), estado.getNomeEstado(), String.valueOf(indice.getAno()));
            List<Double>indiceRegiao = regiao.calcularIDHRegiao(indice.getAno());
            ds.addValue(indiceRegiao.get(0), regiao.getNomeRegiao(), String.valueOf(indice.getAno()));
            List<Double>indicePais = controle.calcularIDHTotalPais(indice.getAno());
            ds.addValue(indicePais.get(0),"Brasil", String.valueOf(indice.getAno()));
        }
        return ds;
